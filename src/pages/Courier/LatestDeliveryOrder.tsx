import {
  ...
  import { CouierDeliveryStatus } from "./CouierDeliveryStatus";
  ...
  const LatestDeliveryOrder = () => {
      ...
      getPersonalProfile(token)
        .then((res) => {
          ...
          deliveryByCouier(token)
            .then((res) => {
              let courierDeliverypage = res.data.filter(
                (c: any) => c.status !== "DELIVERED"
              );
              setOrders(courierDeliverypage);
              return;
            })
            .catch((error) => {
              console.log(error);
    ...
    return (
      <div className="pt-5">
        <h1 className="text-2xl font-bold text-red-700 p-3 uppercase  bg-gray-200 text-center">
          Latest Order
        </h1>
  ...
  export default LatestDeliveryOrder;